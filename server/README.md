...
* Receives a json form data of:
    ...
    3. `email`, string: the email address of the new user
    4. `phone_number`, string: the phone number of the new user
...
* Else, return a json of:
    ...
    4. `is_deleted`, bool: whether the account has been deleted or not
    5. `last_login`, string: the time when this account was last logged in, sample: "2022-03-13 04:15:52"
    6. `password`, string: always set to "\[HIDDEN\]"
    7. `phone_number`, string: the phone number of the user
    8. `uid`, integer: a unique id of the user
    9. `username`, integer: the username of the user
...
* Receives a json form data of:
    ...
    4. `fullname_public`, bool: whether the user's fullname will be shown when others examine this image.
    5. `email_public`, bool: whether the user's email will be shown when others examine this image.
    6. `phone_public`, bool: whether the user's phone will be shown when others examine this image.
    7. `time_public`, bool: whether the upload time of this image will be shown when others examine this image.
...
* Else, return a json of:
    ...
    7. `id`, integer: a unique id of the image (DIFFERENT from tag).
    8. `imgname`, string: the name of the image.
    9. `message`, string: the enclosed message.
    10. `message_encrypted`, bool: true if the message is cyphertext, else false.
    11. `owner`, string: the username of the creator.
    12. `phone`, string: the phone number of the creator. Empty string if the creator didn't disclose this info.
...
* ### Back-end Endpoint: GET /images/get_tag/\<int:id\>/
This URL returns the public information of an image with the image id. The return value of this endpoint is exactly the same as the endpoint `/images/get_tag/<string:tag>/`. The only difference is that the argument in the url should be the id of the image, not the tag.
...
* Return a json that has only one element called `result` in it. The value of the result is an array of json. Each element of that array is in the form of:
    1. `image`, json of the form:
        ...
        6. `id`, integer: a unique id of the image (DIFFERENT from tag).
    ...
    2. `requests`, an array of json. Each element of that array is in the form:
        ...
        3. `imgid`: the id of the image that this request was sent to.
        4. `is_deleted`: whether this request was removed or not.
        5. `message`: the request message.
        6. `status`: status of the request: "REJECTED", "AUTHORIZED" or "PENDING".
        7. `username`: the username of the requester.
...
* ### Back-end Endpoint: GET /images/my_creation/\<int:imgid\>/
...
* Receives the id of the image from the url `imgid` field (NOT from a json form). This should be the id of the image, not the tag stored in the image
* If the user hasn't logged in, then abort `403`
...
* The return value is a json of the form
    1. `image`, json of the form:
        ...
        11. `phone`, string: the phone number of the creator. Empty string if the creator didn't disclose this info.
        12. `time`, string: the time when the info of image was stored, sample: "2022-03-13 04:15:52". Empty string if the creator didn't disclose this info.
    2. `requests`, a json of the form:
        1. `created`: the time when this request was created, sample: "2022-03-13 04:15:52".
        2. `id`: the request id of this request.
        3. `imgid`: the id of the image that this request was sent to.
        4. `message`: the request message.
        5. `status`: status of the request: "REJECTED", "AUTHORIZED" or "PENDING".
...
* Receives a json form data of:
    ...
    2. `action`, string: "AUTHORIZED" or "REJECTED"
* If the user hasn't logged in, then abort `403`.
* If the request with the id doesn't exist or wasn't sent to the user, abort `404`
...
* Receives a json form data of:
    1. `imgid`, integer: the id of the image requested (NOT the tag of the image).
